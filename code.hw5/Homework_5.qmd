...
```{r pair-plot}
sarr_subset %>% 
  #selects columns from species to num_phylls, which only excludes totmass (which is the resonse)
...
#null linear model which assumes no relationship, with data from the updated frame without missing values
null <- lm(totmass ~ 1, data= sarr_subset)
#full linear model which assumes a relationshup with all the variables, with data from the updated frame without missing values
...
```{r Viz_Check}
#plots have 2 rows and 2 columns
par(mfrow = c(2,2))
...
#tansform the linear models via a log transformation
...
#plotting the transformed model, while also checking for normality and homoscedasticity
plot(full_log)
...
```{r new_Models}
#created new models (with log transformations) using different predictor variables, all from the data frame without missing values
...
```{r Check}
#plotted all the models and checked for normality and homoscesdasticity
plot(model_1)
...
```{r}
summary(full_log)
tidy(full_log, conf.int = TRUE, conf.level = .95)
```

c\.

```{r}
#model_predict is a new object which is used to back transform estimates using ggpredict
model_predict <- ggpredict(full_log, terms= "num_lvs", back.transform = TRUE)
summary(model_predict)
#plot model_predict
plot(model_predict, add.data=TRUE)
```
